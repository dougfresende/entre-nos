...
function Organizer() {
  ...
  return <main className="organizer-shell"><header className="organizer-header"><a href="/" className="wordmark">Entre Nós<span>painel dos noivos</span></a><span className="preview-badge">PRÉVIA LOCAL</span></header><div className="organizer-page"><div className="section-eyebrow">MARINA & RAFAEL · 17 DE SETEMBRO</div><h1>O dia, visto de perto.</h1><p className="organizer-lead">Um lugar calmo para acompanhar o que chega e preparar cada detalhe.</p><section className="organizer-stats"><article><span>MEMÓRIAS RECEBIDAS</span><strong>24</strong><small>+8 desde ontem</small></article><article><span>RECADOS RESERVADOS</span><strong>11</strong><small>3 aguardam revisão</small></article><article><span>ETIQUETAS ATIVAS</span><strong>12</strong><small>todas em uso</small></article></section><section className="organizer-grid"><article className="organizer-card tag-card"><div><p className="section-eyebrow">NFC + QR</p><h2>As portas para o nosso dia.</h2><p>Todos os cartões levam ao mesmo destino e podem ser revogados individualmente.</p></div><div className="tag-row"><span className="tag-dot">⌁</span><span><strong>Mesa Jardim</strong><small>/t/mesa-jardim</small></span><b>Ativa</b></div><div className="tag-row"><span className="tag-dot">⌁</span><span><strong>Mesa das Flores</strong><small>/t/mesa-flores</small></span><b>Ativa</b></div><button className="outline-action" onClick={() => setNotice('O gerador de QR será liberado após configurar o domínio HTTPS real.')}>Preparar cartões <Icon name="arrow" /></button></article><article className="organizer-card moderation-card"><p className="section-eyebrow">MODERAÇÃO</p><h2>O que chegou hoje.</h2><div className="moderation-line"><span className="moderation-thumb olive">08</span><span><strong>8 fotos e vídeos</strong><small>aguardando revisão</small></span><button onClick={() => setNotice('A fila de revisão será conectada à API de mídia.')}>Revisar</button></div><div className="moderation-line"><span className="moderation-thumb warm">03</span><span><strong>3 recados</strong><small>reservados para abrir depois</small></span><button onClick={() => setNotice('Os recados permanecem privados até a abertura definida pelo casal.')}>Abrir</button></div><div className="privacy-note">Conteúdo privado só aparece para os noivos após aprovação.</div></article></section>{notice && <p className="organizer-notice" role="status">{notice}</p>}</div></main>
}
...
function App() {
  ...
  if (window.location.pathname === '/organizar') return <Organizer />
